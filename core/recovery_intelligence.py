...
class RecoveryIntelligence:
    ...
    def _choose_recovery_action(self, strategy: StrategyType, 
                               market_context: MarketContext, num_positions: int) -> ActionType:
        """
        🔄 เลือกการกระทำแก้ไม้
        
        หน้าที่:
        - เลือกการกระทำตามประเภทกลยุทธ์แก้ไม้
        - พิจารณาสภาวะตลาดและตำแหน่งปัจจุบัน
        """
        if strategy == StrategyType.CONSERVATIVE_MARTINGALE:
            # เพิ่มตำแหน่งแบบระมัดระวัง
            if market_context.trend_strength > 50:  # เทรนด์ขึ้น
                return ActionType.RECOVERY_BUY
            else:  # เทรนด์ลงหรือไซด์เวย์
                return ActionType.RECOVERY_SELL
        
        elif strategy == StrategyType.AGGRESSIVE_GRID:
            # เพิ่มตำแหน่งแบบกริด
            if num_positions % 2 == 0:  # เปิดสลับทิศทาง
                return ActionType.RECOVERY_BUY
            else:
                return ActionType.RECOVERY_SELL
        
        elif strategy == StrategyType.HEDGING_RECOVERY:
            # เปิด hedge
            return ActionType.HEDGE
        
        elif strategy == StrategyType.BREAKOUT_RECOVERY:
            # ตามทิศทาง breakout
            if market_context.trend_strength > 70:
                return ActionType.RECOVERY_BUY
            elif market_context.trend_strength < -70:
                return ActionType.RECOVERY_SELL
            else:
                return ActionType.HOLD
        
        elif strategy == StrategyType.MEAN_REVERSION:
            # ตรงข้ามกับเทรนด์ปัจจุบัน
            if market_context.trend_strength > 50:
                return ActionType.RECOVERY_SELL  # ขายเมื่อขึ้นมาก
            else:
                return ActionType.RECOVERY_BUY   # ซื้อเมื่อลงมาก
        ...
        elif strategy == StrategyType.NEWS_BASED:
            # ตัดสินใจตามข่าว
            if market_context.news_impact_level >= 3:
                if market_context.volatility_score > 70:
                    return ActionType.RECOVERY_BUY  # ขึ้นตามข่าวดี
                else:
                    return ActionType.HOLD  # รอให้ชัด
            else:
                return ActionType.HOLD
        ...
        # เพิ่มข้อจำกัดเพิ่มเติม
        if num_positions >= self.max_positions:
            if action in [ActionType.BUY, ActionType.SELL, ActionType.RECOVERY_BUY, ActionType.RECOVERY_SELL]:
                action = ActionType.HOLD
                confidence = 0.3
                reasoning.append("Maximum positions reached")
                warnings.append("Cannot open new positions - limit reached")
